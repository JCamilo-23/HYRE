...
const quickActions = [
  {
    ...
    color: "bg-[var(--hyre-pink)]/20 text-[var(--hyre-pink)]",
  },
  {
    icon: Zap,
    label: "Simulacion",
    ...
    color: "bg-[var(--hyre-cyan)]/20 text-[var(--hyre-cyan)]",
  },
  {
    icon: Video,
    label: "Entrevista IA",
    description: "Practica con IA en tiempo real",
    screen: "interview" as Screen,
    color: "bg-[var(--hyre-green)]/20 text-[var(--hyre-green)]",
  },
]
  ...
  return (
    <div className="min-h-screen pb-24 px-4 pt-6 safe-area-top">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <button className="relative w-10 h-10 rounded-full glass flex items-center justify-center">
            ...
            <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-[var(--hyre-pink)] rounded-full" />
          </button>
          <button 
            onClick={() => onNavigate("profile")}
            className="w-10 h-10 rounded-full bg-gradient-to-br from-primary to-[var(--hyre-pink)] flex items-center justify-center text-white font-semibold"
          >
            C
          </button>
        </div>
        ...
        {stats.map((stat) => (
          ...
          >
            <Card className="glass p-3 border-0">
              ...
              <span className="text-xs text-[var(--hyre-green)]">{stat.change}</span>
            </Card>
          </motion.div>
        ))}
      </div>
        ...
        {quickActions.map((action) => (
          ...
          >
            ...
            >
              ...
              <p className="text-[10px] text-muted-foreground mt-1 leading-tight">{action.description}</p>
            </Card>
          </motion.div>
        ))}
      </div>
      ...
      <motion.div
        ...
        className="mb-6"
      >
        <Card className="glass-strong p-4 border-0 bg-gradient-to-r from-primary/20 to-[var(--hyre-cyan)]/20">
