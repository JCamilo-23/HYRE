...
export function RegisterScreen({ userType, onComplete }: RegisterScreenProps) {
  ...
  const [loadingProvider, setLoadingProvider] = useState<ExternalSocialProvider | null>(null)
  ...
  const handleSocialLogin = (provider: ExternalSocialProvider) => {
    setLoadingProvider(provider)
    setRedirectMessage(`Te llevamos a ${getProviderLabel(provider)} para iniciar sesion...`)
    startExternalSocialLogin({ provider, userType })
  }

  const socialButtonsDisabled = isLoading || loadingProvider !== null
  ...
  if (mode === "options") {
    return (
      <div className="min-h-screen flex flex-col px-6 py-8 safe-area-top safe-area-bottom">
        ...
        {redirectMessage && (
          <p className="text-[#94A3B8] text-sm text-center mb-2">{redirectMessage}</p>
        )}
